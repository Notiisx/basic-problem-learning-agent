...
def get_summary() -> dict:
    ...
    sessions = _load()
    if not sessions:
        return {"total": 0, "solved": 0, "solve_rate": 0, "by_topic": {}}
...
def get_recent_solve_rate(n: int = 10) -> float:
    """Solve rate over the last n sessions."""
    sessions = _load()
    if not sessions:
        return 0.0
    recent = sessions[-n:]
    return round(sum(1 for s in recent if s["solved"]) / len(recent), 2)
